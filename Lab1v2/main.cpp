...
class AStar {
public:
    Result find_path(Node start, Node goal, Map grid, std::string metrictype = "Octile", int connections = 8, double hweight = 1) { //создаем метод find path, имеет значения по умолч,
        auto time_now = std::chrono::high_resolution_clock::now();
